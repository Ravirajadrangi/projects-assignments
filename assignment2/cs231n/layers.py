...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    #############################################################################
    #minibatch = np.random.random_integers(0,N-1,int(N/5))
    #sample = x[[minibatch],:]
    #sample = sample.reshape([int(N/5),D])
    sample_mean = x.mean(axis=0)
    sample_var = x.var(axis=0)
    sample_std = sample_var**0.5
    out1 = (x-sample_mean)/(sample_std+eps)
    out = out1*gamma+beta
    ...
    cache['out1'] = out1
    cache['gamma'] = gamma
    cache['sample_std'] = 1/(sample_std+eps)
...
def batchnorm_backward(dout, cache):
  ...
  dx = dx_hat * (1/np.sqrt(cache['sample_var'] + cache['eps'])) + dsample_var * (2*(cache['x']-cache['sample_mean'])/m) + dsample_mean/m


  dgamma = (dout*(cache['out1'])).sum(axis=0)
