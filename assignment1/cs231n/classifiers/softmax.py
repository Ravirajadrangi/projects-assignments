...
def softmax_loss_naive(W, X, y, reg):
  ...
  #############################################################################
  XW = X.dot(W) # (N,C)
  XW_esum = np.exp(XW).sum(axis=1)
  XW_ysum = np.array([XW[i,y[i]] for i in range(N)])
  loss = -XW_ysum + np.log(XW_esum)
  loss = loss.sum()/N + reg*(W**2).sum()/2
  ...
  for i in range(N):
    ...
    for j in range(C):
        ...
        xi_c = (y[i]==j) - np.exp(xi.dot(W[:,j]))/(np.exp(xi.dot(W)).sum())
        xis.append(xi*xi_c)
    dW += np.array(xis).T
  dW = -dW/N + reg*W
  '''
